...
Dotsmith only **decrypts**; it does not encrypt files. Encrypt files yourself with the standalone `age` (or `rage`) tool to the recipient derived from your identity file, then commit the resulting `.age` file.
...
Configuration is a **user-level** concern, read only from the locations below. A `.dotsmith.yml`
inside the dotfiles repository is **never** read — an untrusted/cloned repo must not be able to
redirect the age identity or the compile/target directories on the user's machine. The first
existing file wins:
...
CLI flags override the loaded file. `dotsmith init` scaffolds the config at location 2.
...
| `dotsmith decrypt <file>` | Decrypt an `.age` file and print the plaintext to stdout. Does not modify the original file. Requires an age identity file. (Encryption is done out-of-band with the standalone `age` tool.) |
...
- A state file (`.dotsmith.state`) containing non-local (`..` or absolute) source/target paths: hard error — refuse to use it rather than delete files outside the target directory.
...
| Decryption    | age        | `filippo.io/age` Go library. Identity-file-based decryption of `.age` files during compile. |
...
- **State file:** Test read, write, staleness detection, corruption handling, and rejection of non-local (`..`/absolute) source/target paths.
- **Decryption:** Test identity-file-based decryption, compile with mixed encrypted/unencrypted subfiles, and failure modes (wrong key, corrupt file, missing identity file).
...
- **Decrypt command:** verify stdout decryption and round-trip integrity with identity file mode.
